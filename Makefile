...
test-example: ## Run tests for a specific example (make test-example EXAMPLE=CLICKEDIT)
	@if [ -z "$(EXAMPLE)" ]; then \
		...
		echo "Available examples: ACTIVESEARCH, VALUESELECT, PLY3, PROGRESSBAR, CLICKEDIT, CLICKLOAD, DELETEROW, BULKUPDATE, EDITROW, LAZYLOAD, INLINVALIDATION, FILEUPLOAD, FILEUPLOADPRESERVE, RESETINPUT, DIALOGS"; \
		exit 1; \
	fi
