...
def load_car_data():
    ...
    try:
        ...
        print(f"Loaded {len(mmdb)} car makes with {sum(len(models) for models in mmdb.values())} total models")
...
def getmodels():
    """
    HTMX endpoint that returns HTML fragment of model options for a selected make.
...
def index():
    ...
    return flask.render_template('index.html', models=initial_models, makers=makers)
